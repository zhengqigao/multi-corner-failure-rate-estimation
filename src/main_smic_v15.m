...
% debug(4): 1: use missing value formulation. Note: not used anymore.
...
rng(2029,'twister');  % [38],1689 [2029] for no miss 5e^3 
...
if debug(2)
    for i=1:1:K
        ...
        sigma_g = sigma_list_g(i);
        sram = get_tech_param_sram_smic_v2(0.065,corner(i));
        [ Sqd_temp,sigma2d_temp ] = generate_Sqd_smic( sram,'access',z_list,d_list,5e3 );
        Sqd(i)={Sqd_temp};
...
while count<iter_max
    ...
    for k=1:1:K
        for d=1:1:D
            ...
            % process missing value: not used anymore
            cur_miss = Miss(k,d);
            if cur_miss ~= 0
                Ak(cur_miss,:) = [];
...
for k =1:1:K
    ...
    legend('MC','CBI 1','CBI 2','CBI 3','CBI 4','CBI 5','CBI 6','CBI 7','CBI 8');
...
if debug(5)
    for k=1:K
        ...
        legend('MC','CBI 1','CBI 1up','CBI 1low',...
            'CBI 2','CBI 2up','CBI 2low');
        %set(gca,'xscale','log');
        set(gca,'yscale','log');
